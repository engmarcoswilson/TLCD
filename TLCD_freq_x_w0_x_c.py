...
ceq_max = max(ceq)
ceq_ind = np.argmax(ceq)
ceq_gao_max = max(ceq_gao)
ceq_gao_ind = np.argmax(ceq_gao)
ceq_max = float("{:.3f}".format(ceq_max))
ceq_gao_max = float("{:.3f}".format(ceq_gao_max))
Omg_exc[ceq_ind] = float("{:.3f}".format(Omg_exc[ceq_ind]))
Omg_exc[ceq_gao_ind] = float("{:.3f}".format(Omg_exc[ceq_gao_ind]))

plt.figure(figsize=(12,8))
plt.plot(Omg_exc, ceq, label='ceq')
plt.plot(Omg_exc, ceq_gao, label='ceq_gao')
plt.plot(Omg_exc[ceq_ind], ceq_max, 'o')
plt.text(Omg_exc[ceq_ind]-0.05, ceq_max-0.01, (Omg_exc[ceq_ind], ceq_max), color='blue')
plt.plot(Omg_exc[ceq_gao_ind], ceq_gao_max, 'o')
plt.text(Omg_exc[ceq_gao_ind]-0.05, ceq_gao_max+0.01, (Omg_exc[ceq_gao_ind], ceq_gao_max), color='orange')
